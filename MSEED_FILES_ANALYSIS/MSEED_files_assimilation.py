        ...
        for n in local_networks:
            ...
            for s in local_stations:
                ...
                for c in channels:
                    ...
                    #path of all local files 
                    data_path = f'{local_data_path}/{y}/{n}/{s}/{c}.D'
                    ...
                    #all seiscomp files
                    seiscomp_files = os.listdir(f'{seiscomp_data_path}/{y}/{n}/{s}/{c}.D')
                    #initialization of file_extension variable
                    file_extension = ''

                    #assuming that all files have same extensions (skipping if there are no files)
                    if local_files == [] and seiscomp_files == []:
                        print(f'LOCAL OR SEISCOMP DATA NOT AVAILABLE', file=txt_file)
                        file_extension = ''
                        continue
                    elif local_files == []: #no local files, no need to rename unexisting files
                        print(f'LOCAL DATA NOT AVAILABLE', file=txt_file)
                        file_extension = ''
                    else: #get file_extension or renaming files and status update
                        file_extension = local_files[0].split('.')[-1]

                    #msd or mseed or MSEED are valid file extensions -> files need to be processed
                    if 'msd' in file_extension or 'mseed' in file_extension or 'MSEED' in file_extension:
                        #firstly rename local mseed files and fix their stats
                        uppercase_file_name(file_extension,data_path)
                        mseed_status_update(s,n,data_path)
